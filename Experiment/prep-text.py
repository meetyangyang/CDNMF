...
import text.util
import text.DocLemmatizer#新增DocLemmatizer
import re
import csv
...
def main():
    ...
    parser.add_option("--df", action="store", type="int", dest="min_df", help="minimum number of documents for a term to appear", default=3)
    ...
    for in_path in args:
        ...
        # validation_docs =[]                         # generate whole doc -> for validation use
        # year = []

        for doc_id, body in docgen:
            docs.append(body)   
            # validation_docs.append(body)            # generate whole doc -> for validation use
            # year.append(re.search('\d\d\d\d',in_path).group(0))
            doc_ids.append(doc_id)  
        log.info( "Found %d documents to parse" % len(docs) )


        #######新增单词的Lemmatizer
        docs=text.DocLemmatizer.lemmatizing(docs)
        #######
    ...
    # (validation_X,validation_terms) = text.util.preprocess( validation_docs, stopwords, min_df = options.min_df, apply_tfidf = options.apply_tfidf, apply_norm = options.apply_norm, ngram_range = (1,options.max_ngram) )  
    # validation_out_prefix = 'cce_out_result/validation'
    # text.util.save_corpus( validation_out_prefix, validation_X, validation_terms, doc_ids )
    # from sklearn.externals import joblib
    # matrix_outpath = 'cce_out_result/corpus'
    # log.info( "Saving document to %s" % matrix_outpath )
    # joblib.dump((validation_docs), matrix_outpath ) 

        # rows = zip(validation_docs, year)             # generate whole doc -> for validation use           

        # file_name = './cce_out_result/corpus' + '.csv'
        # f = open(file_name, 'a')
        # w = csv.writer(f)
        # w.writerows(rows)
        # f.close
