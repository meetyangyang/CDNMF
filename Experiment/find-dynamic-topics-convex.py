...
import text.util
import unsupervised.cnmf, unsupervised.rankings, unsupervised.coherence #修改
import joblib
# --------------------------------------------------------------
...
def main():
	...
	parser.add_option("-a","--aW", action="store", type="float", dest="alpha_W", help="initial alpha_W", default=0.0) #改进
	parser.add_option("-b","--aH", action="store", type="float", dest="alpha_H", help="initial alpha_H", default=0.0) #改进
	parser.add_option("-l","--lr", action="store", type="float", dest="l1_ratio", help="initial l1_ratio", default=1.0) #改进
	...
	for window_model_path in args:
		...
		(doc_ids, terms, term_rankings, partition, W, H, window_topic_labels) = unsupervised.cnmf.load_nmf_results( window_model_path ) #修改
	...
	# CNMF implementation #修改
	impl = unsupervised.cnmf.SklNMF( max_iters = options.maxiter, init_strategy = "nndsvd", random_seed = random_seed, alpha_W = options.alpha_W, alpha_H = options.alpha_H, l1_ratio = options.l1_ratio) #改进
	...
	for k in range(kmin,kmax+1):
		...
		# Write results
		results_out_path = os.path.join( dir_out, "dynamictopics_convex_k%02d.pkl"  % (k) ) #改进
		if not os.path.exists(dir_out+'\\model_save\\'): #改进
			os.makedirs(dir_out+'\\model_save\\')
		unsupervised.cnmf.save_nmf_results( results_out_path, collection.topic_ids, all_terms, term_rankings, partition, impl.W, impl.H, topic_labels )
		# joblib.dump(impl.model,'F:\\Desktop\\model_save\\dynamic_model'+str(k)+'.model')
		joblib.dump(impl.model, dir_out+'\\model_save\\dynamic_convex_model'+str(k)+'.model') #改进

	# filepath='F:\\Desktop\\model_save\\dynamic_modelcoherence.csv'
	filepath=dir_out+'\\model_save\\dynamic_convex_modelcoherence.csv' #改进
	with open(filepath, "w") as fout:
		fout.write("k,coherence\n")
		for k in range(kmin,kmax+1):
			fout.write("%s,%s\n" % ( k, coherence_scores[k] ) )
	fout.close()
