...
def main():
	...
	if options.model_type == "cbow":
		model = gensim.models.Word2Vec(docgen, vector_size=options.dimensions, min_count=options.min_df, window=options.w2v_window, workers=4, sg = 0)
	elif options.model_type == "sg":
		model = gensim.models.Word2Vec(docgen, vector_size=options.dimensions, min_count=options.min_df, window=options.w2v_window, workers=4, sg = 1)
	else:
